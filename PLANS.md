...
- [ ] Run `topo2ifc --input tests/fixtures/sbco_minimal.ttl --output /tmp/sbco_test.ifc` and confirm IFC is produced without errors.
- [ ] Add an end-to-end test (or extend `tests/test_end_to_end.py`) for the SBCO fixture.
...
- [ ] Parse `sbco:Level` (and `BOT:Storey`) nodes to attach a storey elevation to each contained space.
- [ ] Store the level/elevation on `SpaceSpec` (new optional field `storey_elevation`).
- [ ] Use `SpaceSpec.storey_elevation` in `ifc/exporter.py` to place spaces on the correct `IfcBuildingStorey`.

### 2-2 Multi-storey IFC export
- [ ] Extend `IfcExporter` to create one `IfcBuildingStorey` per distinct storey elevation found in the layout.
- [ ] Assign each `IfcSpace` to its storey.
- [ ] Add tests in `tests/test_end_to_end.py` for a two-storey fixture.
...
### 4-2 Documentation
- [ ] Update `README.md` to add an SBCO-vocabulary example and link to the smartbuilding_ontology repository.
...
### 4-3 CI pipeline
- [ ] Add a GitHub Actions workflow (`.github/workflows/ci.yml`) that runs `pytest tests/` on every push and pull request.
